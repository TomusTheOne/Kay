...
/* ------------------------------------------------------------ formatting --
   French, because that is who reads the admin. Money is in pesos, the
   currency the card is charged in and the till counts.                     */

/** 12 320 MXN, with a narrow no-break space so the figure never wraps. */
function kay_mxn(int|float $pesos, bool $unit = true): string
{
    return number_format((float) $pesos, 0, ',', "\u{202F}") . ($unit ? "\u{00A0}MXN" : '');
}

function kay_usd(int|float $dollars): string
{
    return number_format((float) $dollars, 0, ',', "\u{202F}") . "\u{00A0}USD";
}
...
{
    return number_format($ratio * 100, $decimals, ',', "\u{202F}") . "\u{00A0}%";
}
...
function kay_compact(int $n): string
{
    ...
    return number_format($n / 1000, $n % 1000 === 0 ? 0 : 1, ',', '') . "\u{00A0}k";
}

function kay_fmt_date(string $ymd, string $pattern): string
{
    ...
    if (class_exists(IntlDateFormatter::class)) {
        $fmt = new IntlDateFormatter('fr_FR', IntlDateFormatter::FULL, IntlDateFormatter::NONE, kay_tz(), null, $pattern);
        $out = $fmt->format($date);
...
/** "mar. 1 déc.", with the year only when it is not this year. */
function kay_date_short(string $ymd): string
{
    $sameYear = substr($ymd, 0, 4) === kay_now()->format('Y');
    return kay_fmt_date($ymd, $sameYear ? 'EEE d MMM' : 'EEE d MMM y');
}

function kay_date_long(string $ymd): string
{
    return kay_fmt_date($ymd, 'EEEE d MMMM y');
}
...
/** "3 oct. à 14:05", or "aujourd’hui à 14:05". */
function kay_when(?string $sql): string
{
    ...
    $day = $t->format('Y-m-d');
    $label = match ($day) {
        kay_today()                                   => 'aujourd’hui',
        kay_now()->modify('-1 day')->format('Y-m-d')  => 'hier',
        default                                       => kay_fmt_date($day, 'd MMM' . ($t->format('Y') === kay_now()->format('Y') ? '' : ' y')),
    };
    return $label . ' à ' . $t->format('H:i');
}

/** The site's own product names, in French. */
function kay_product_name(string $slug): string
{
    return (string) (kay_messages('fr')['products']['items'][$slug]['name'] ?? $slug);
}

function kay_dives_text(int $dives): string
{
    return match (true) {
        $dives <= 0  => 'demi-journée',
        $dives === 1 => '1 plongée',
        default      => $dives . ' plongées',
    };
}

function kay_pickup_name(string $slug): string
{
    return (string) (kay_messages('fr')['logistics']['pickups'][$slug]['name'] ?? $slug);
}
...
function kay_slot_text(string $slot, string $note = ''): string
{
    ...
    return $note === '' ? 'Horaire à fixer' : 'Horaire à fixer (' . $note . ')';
}

function kay_status_badge(string $status): string
{
    return '<span class="badge badge--' . kay_h($status) . '">' . kay_h(KAY_STATUSES[$status] ?? $status) . '</span>';
}

/** "🇨🇦 Canada" from 'CA'. The flag is two regional-indicator letters. */
function kay_country(string $cc): string
{
    if (!preg_match('/^[A-Z]{2}$/', $cc)) {
        return 'Inconnu';
    }
    $name = class_exists(Locale::class) ? (string) Locale::getDisplayRegion('-' . $cc, 'fr') : '';
...
function kay_page_start(string $title, string $current, ?array $admin): void
{
    ...
    if ($admin !== null) {
        echo '<a class="skip" href="#main">Aller au contenu</a>'
        ...
        foreach (KAY_NAV as $page => [$label, $short, $icon]) {
            echo '<a href="' . $page . '"' . ($page === $current ? ' aria-current="page"' : '') . '>'
                . kay_icon($icon) . '<span class="nav__long">' . kay_h($label) . '</span>'
                . '<span class="nav__short">' . kay_h($short) . '</span></a>';
        }
        echo '</nav><div class="side__foot">'
            ...
            . '<a href="/" class="side__site" target="_blank" rel="noopener">Voir le site ↗</a>'
            . '</div></header>';
    }

    echo '<main id="main" class="main">';
    if ($flash !== null) {
        echo '<p class="flash" role="status">' . kay_h($flash) . '</p>';
    }
}
...
/** « Précédent · 2 / 7 · Suivant », keeping every other filter. */
function kay_pager(string $page, array $query, int $total, int $current, int $per): string
{
    ...
    $out .= $current > 1
        ? '<a class="btn btn--ghost" href="' . kay_h(kay_url($page, ['p' => $current - 1] + $query)) . '">← Précédent</a>'
        : '<span></span>';
    $out .= '<span class="muted">Page ' . $current . ' / ' . $pages . '</span>';
    $out .= $current < $pages
        ? '<a class="btn btn--ghost" href="' . kay_h(kay_url($page, ['p' => $current + 1] + $query)) . '">Suivant →</a>'
        : '<span></span>';
    return $out . '</nav>';
}

/* -------------------------------------------------------------------- csv --
   Semicolons and a BOM, so a double-click opens it correctly in a French
   Excel. Cells a spreadsheet would run as a formula are defused: the names
...
function kay_csv(string $filename, array $header, iterable $rows): never
{
    ...
    fputcsv($out, $header, ';', '"', '');
    foreach ($rows as $row) {
        fputcsv($out, array_map('kay_csv_cell', $row), ';', '"', '');
    }
    fclose($out);
    exit;
}
...
function kay_chart_columns(array $points, array $o): string
{
    ...
    $table = '';
    if (!$mini) {
        $table = '<details class="chart__data"><summary>Voir les données</summary><div class="table-wrap">'
            . '<table class="table table--compact"><thead><tr><th scope="col">Période</th>'
...
function kay_tile(string $label, string $value, ?int $now = null, ?int $before = null, string $note = ''): string
{
    ...
    if ($now !== null && $before !== null) {
        if ($before === 0) {
            $delta = $now > 0 ? '<span class="delta delta--up">▲ nouveau</span>' : '';
        } else {
...
function kay_booking_fields(array $v, bool $withContact): string
{
    ...
    $out = '<div class="grid">';
    $out .= '<label class="field field--wide"><span>Sortie</span><select name="item" data-quote-item required>';
    ...
    $out .= '<label class="field"><span>Date</span><input type="date" name="date" value="'
        . kay_h($v['date'] ?? '') . '" required></label>';

    $out .= '<label class="field"><span>Départ</span><select name="slot">';
    foreach ($cat['schedules'] as $s) {
        $out .= '<option value="' . kay_h($s['slug']) . '"' . (($v['slot'] ?? '0800') === $s['slug'] ? ' selected' : '') . '>'
            . kay_h($s['start'] !== null ? $s['start'] . ' – ' . $s['end'] : 'Autre horaire') . '</option>';
    }
    $out .= '</select></label>';

    $out .= '<label class="field"><span>Horaire souhaité <small>(si autre)</small></span><input name="slotNote" maxlength="120" value="'
        . kay_h($v['slotNote'] ?? '') . '"></label>';

    $out .= '<label class="field"><span>Plongeurs</span><input type="number" name="divers" min="1" max="8" value="'
        . (int) ($v['divers'] ?? 1) . '" data-quote-divers required></label>';

    $out .= '<label class="field"><span>Transport</span><select name="pickup" data-quote-pickup>';
    ...
    $out .= '<label class="field"><span>Niveau</span><input name="cert" list="certs" maxlength="64" value="'
        . kay_h($v['cert'] ?? '') . '"></label><datalist id="certs">';
    foreach ((array) ($fr['book']['certs'] ?? []) as $cert) {
        $out .= '<option value="' . kay_h($cert) . '">';
    }
    $out .= '</datalist>';

    $out .= '<label class="field"><span>Nom du client</span><input name="name" maxlength="160" value="'
        . kay_h($v['name'] ?? '') . '" required></label>';
    $out .= '<label class="field"><span>E-mail <small>(facultatif)</small></span><input type="email" name="email" value="'
        . kay_h($v['email'] ?? '') . '"></label>';

    if ($withContact) {
        $out .= '<label class="field"><span>Téléphone / WhatsApp</span><input type="tel" name="phone" maxlength="40" value="'
            . kay_h($v['phone'] ?? '') . '" placeholder="+52 …"></label>';
        $out .= '<label class="field"><span>Langue des e-mails</span><select name="locale">';
        foreach (['en' => 'Anglais', 'es' => 'Espagnol', 'fr' => 'Français'] as $code => $label) {
            $out .= '<option value="' . $code . '"' . (($v['locale'] ?? 'en') === $code ? ' selected' : '') . '>' . $label . '</option>';
        }
        $out .= '</select></label>';
    }
    $out .= '</div>';

    $out .= '<p class="quote" data-quote aria-live="polite"></p>'
...
{
    if ($notes === []) {
        return '<p class="empty">Rien pour l’instant.</p>';
    }
    ...
    foreach ($notes as $n) {
        ...
        $out .= '<li class="timeline__item timeline__item--' . kay_h($n['kind']) . ($done ? ' is-done' : '') . '">'
            ...
            . kay_h(KAY_NOTE_KINDS[$n['kind']] ?? $n['kind']) . '</span> '
            . kay_h(kay_when($n['created_at'])) . ($n['author'] ? ' · ' . kay_h($n['author']) : '')
            . ($task ? ' · pour le ' . kay_h(kay_date_short((string) $n['due_on'])) : '') . '</p>'
        ...
        if ($task) {
            $out .= '<form method="post" action="note.php">' . kay_csrf_field($admin)
                ...
                . '<button class="btn btn--small ' . ($done ? 'btn--ghost">Rouvrir' : 'btn--primary">Fait') . '</button></form>';
        }
        $out .= '</li>';
    }
    return $out . '</ol>';
...
function kay_customer_fields(array $v): string
{
    ...
    $out = $input('name', 'Nom', 'text', ' maxlength="160" required')
        . $input('email', 'E-mail', 'email')
        . $input('phone', 'Téléphone / WhatsApp', 'tel', ' maxlength="40" placeholder="+1 …"')
        . $input('country', 'Pays <small>(code, ex. CA)</small>', 'text', ' maxlength="2" list="countries" autocapitalize="characters"')
        . '<datalist id="countries">';
    foreach (KAY_COMMON_COUNTRIES as $cc) {
        $out .= '<option value="' . $cc . '">' . kay_h(kay_country($cc)) . '</option>';
    }
    $out .= '</datalist><label class="field"><span>Langue</span><select name="locale">';
    foreach (['en' => 'Anglais', 'es' => 'Espagnol', 'fr' => 'Français'] as $code => $label) {
        $out .= '<option value="' . $code . '"' . (($v['locale'] ?? 'en') === $code ? ' selected' : '') . '>' . $label . '</option>';
    }
    $out .= '</select></label>'
        . $input('certification', 'Niveau', 'text', ' maxlength="64" list="certs"')
        . '<datalist id="certs">';
    foreach ((array) (kay_messages('fr')['book']['certs'] ?? []) as $cert) {
        $out .= '<option value="' . kay_h($cert) . '">';
    }
    $out .= '</datalist>'
        . '<label class="field"><span>Étiquettes <small>(séparées par des virgules)</small></span><input name="tags" value="'
        . kay_h(str_replace(',', ', ', (string) ($v['tags'] ?? ''))) . '" placeholder="vip, nitrox, groupe"></label>'
        . '<label class="field"><span>Notes permanentes</span><textarea name="notes" rows="4" '
        . 'placeholder="Taille de combinaison, allergies, préférences…">' . kay_h($v['notes'] ?? '') . '</textarea></label>';
    return $out;
}
